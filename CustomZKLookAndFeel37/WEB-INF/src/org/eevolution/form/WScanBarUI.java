...
public class WScanBarUI extends ScanBar implements IFormController, EventListener ,ValueChangeListener
{
    ...
    public void onEvent(Event e)
    {
        ...
        if (e.getTarget().equals(lotField.getComponent()))
        {	
			addScanLot();
			return;
        }	
        
        if (e.getTarget().equals(serNoField.getComponent()))
        {	
			addScanSetNo();
			return;
        }	
    ...
    private void setLocator() {
    	if (locatorField.getValue() !=null)
			M_Locator_ID = (Integer)locatorField.getValue();
		else
			M_Locator_ID = 0;
	}
    ...
    private void setLocatorTo() {
    	if (locatorToField.getValue() !=null)
			M_LocatorTo_ID = (Integer)locatorToField.getValue();
		else
			M_LocatorTo_ID = 0;
	}
